...
pub const ArgParser = struct {
    const InnerType = switch (builtin.is_test) {
    ...
    pub fn parse(allocator: std.mem.Allocator, command: []const u8) !Self {
        var iter = switch (builtin.is_test) {
...
test "test real args" {
    ...
    var parser = try ArgParser.parse(allocator, "binary-path test");
    defer parser.deinit();
...
test "check if elements and keys are present" {
    const allocator = std.testing.allocator;
    var parser = try ArgParser.parse(allocator, "binary-path test --bool-arg --string-arg=string-value");
    defer parser.deinit();
